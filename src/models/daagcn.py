...
class DAAGCN(BaseModel):
    """
    Reference code: https://github.com/LeiBAI/AGCRN
    """

    def __init__(self, embed_dim, rnn_unit, num_layer, cheb_k, **args):
        super(DAAGCN, self).__init__(**args)
        self.node_embeddings = nn.Parameter(
            torch.randn(self.node_num, embed_dim), requires_grad=True
        )
        self.time_embeddings = nn.Parameter(
            torch.randn(self.horizon, embed_dim), requires_grad=True
        )
        self.encoder = DAGRNN(
            self.node_num, self.input_dim, rnn_unit, cheb_k, embed_dim, num_layer
        )
        self.layernorm = nn.LayerNorm(rnn_unit, eps=1e-12)
        self.out_dropout = nn.Dropout(0.1)
        self.end_conv = nn.Conv2d(
            1, self.horizon * self.output_dim, kernel_size=(1, rnn_unit), bias=True
        )

    def forward(self, source, label=None, node_embed=None):
        # source: B, T, N, D
        init_state = self.encoder.init_hidden(source.shape[0])
        output, _ = self.encoder(
            source, init_state, node_embed, self.time_embeddings
        )  # B, T, N, hidden
        output = self.out_dropout(
            self.layernorm(output[:, -1:, :, :])
        )  # B, 1, N, hidden

        # CNN based predictor
        output = self.end_conv((output))  # B, T*C, N, 1
        output = output.squeeze(-1).reshape(
            -1, self.horizon, self.output_dim, self.node_num
        )
...
class DAGRNN(nn.Module):
    def __init__(self, node_num, dim_in, dim_out, cheb_k, embed_dim, num_layers=1):
        ...
        for _ in range(1, num_layers):
            self.dagrnn_cells.append(
                GRUCell(node_num, dim_out, dim_out, cheb_k, embed_dim)
            )
...
class GRUCell(nn.Module):
    def __init__(self, node_num, dim_in, dim_out, cheb_k, embed_dim):
        ...
        self.gate = DAGCN(
            dim_in + self.hidden_dim, 2 * dim_out, cheb_k, embed_dim, node_num
        )
        self.update = DAGCN(
            dim_in + self.hidden_dim, dim_out, cheb_k, embed_dim, node_num
        )
    ...
    def forward(self, x, state, node_embeddings, time_embeddings):
        ...
        z_r = torch.sigmoid(
            self.gate(input_and_state, node_embeddings, time_embeddings)
        )
...
class DAGCN(nn.Module):
    ...
    def forward(self, x, node_embeddings, time_embeddings):
        ...
        node_embeddings = self.embs_dropout(
            self.layernorm(node_embeddings + time_embeddings.unsqueeze(0))
        )  # torch.mul(node_embeddings, node_time)
        supports = F.softmax(
            torch.mm(node_embeddings, node_embeddings.transpose(0, 1)), dim=1
        )
