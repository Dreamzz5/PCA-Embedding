...
class graph_constructor(nn.Module):
    ...
    def forward(self, idx, node_embed):
...
class MTGNN(BaseModel):
    ...
    def __init__(self, 
                 ...
                 node_dim=24, dilation_exponential=1, 
                 conv_channels=32, residual_channels=32, 
                 skip_channels=64, end_channels=128, 
                 seq_length=12, in_dim=3, out_dim=12, layers=3, propalpha=0.05, tanhalpha=3, layer_norm_affline=True, **args):
        ...
        self.gc = graph_constructor(
            self.node_num, subgraph_size, node_dim, alpha=tanhalpha, static_feat=static_feat)
        ...
        for i in range(1):
            ...
            for j in range(1, layers+1):
                ...
                #         (residual_channels, self.node_num, self.seq_length - rf_size_j + 1), elementwise_affine=layer_norm_affline))
                # else:
                #     self.norm.append(LayerNorm(
                #         (residual_channels, self.node_num, self.receptive_field - rf_size_j + 1), elementwise_affine=layer_norm_affline))
    ...
    def forward(self, history_data: torch.Tensor, labels: torch.Tensor, node_embed: torch.Tensor,  **kwargs) -> torch.Tensor:
        ...
        if self.gcn_true:
            if self.buildA_true:
                adp = self.gc(self.idx, node_embed)
            else:
