...
class BaseEngine:
    ...
    def load_model(self, save_path):
        ...
        checkpoint = torch.load(os.path.join(save_path, filename), map_location='cpu')
        pretrained_dict = {
            k: v
    ...
    def train_batch(self):
        ...
        for X, label in self._dataloader["train_loader"].get_iterator():
            ...
            node_embed = torch.nn.functional.dropout(node_embed, 0.3, training=True)
            pred = self.model(X, label, node_embed)
    ...
    def train(self):
        ...
        for epoch in range(self._max_epochs):
            ...
            if mvalid_loss < min_loss:
                ...
                if wait == self._patience:
                    self._logger.info(
                        "Early stop at epoch {}, best loss = {:.6f} at epoch {}".format(
                            epoch + 1, min_loss, best_epoch + 1
                        )
                    )
                    break
        ...
        self._logger.info(f"Training finished. Loading best model from epoch {best_epoch + 1}")
        
        # 保存最优模型
        self.save_model(self._save_path)
        self._logger.info(f"Saving best model with loss {min_loss:.4f}")
        
        # 进行测试评估
        self.evaluate("test")

    def evaluate(self, mode, net=None):
        if (mode == "test" or mode == "shift") and net is None:
