...
def calculate_scaled_laplacian(adj_mx, lambda_max=None, undirected=True):
    ...
    L = sp.csr_matrix(L)  # 确保是CSR格式
    
    if lambda_max is None:
        try:
            # 方法1: 增加容差和迭代次数的eigsh
            lambda_max, _ = linalg.eigsh(L, 1, which='LM', tol=1e-3, maxiter=5000)
            lambda_max = lambda_max[0]
        except:
            # 方法2: 如果方法1失败，添加一个小的对角线扰动重试
            L_reg = L + sp.eye(L.shape[0]) * 1e-6
            lambda_max, _ = linalg.eigsh(L_reg, 1, which='LM', tol=1e-3, maxiter=5000)
            lambda_max = lambda_max[0]
